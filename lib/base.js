...
const { INTERNAL_SERVER_ERROR, NOT_FOUND, SEE_OTHER } = require('http-codes');
const { forEach, merge } = require('lodash');
const { Router } = require('express');
const asyncpipe = require('asyncpipe');
const capitalize = require('capitalize');
const createError = require('http-errors');
const { STATUS_CODES: httpCodeMessage } = require('http');
...
module.exports = class BaseAuthenticator {
  ...
  attachModels() {
    ...
    this.models = {};
    forEach(this.character.db.models, (model, name) => {
  ...
  findIdentity(account) {
    ...
    const { Authentication$Account, Core$Identity } = this.character.db.models;
